...
class Channel{
	...
	public static function init(){
		...
		$query = Database::getInstance()->query("SELECT * FROM " . table("channel"));
	...
	public static function remove(ChannelData $channel){
		...
		($database = Database::getInstance())->query("DELETE FROM " . table("channel") . " WHERE `id`='" . $channel->id() . "'");
	...
	public static function join($name, UserData $user, MessageParser $message = null){
		...
		if(($channel = self::get($name)) == null){
			...
			$query = Database::getInstance()->query("SELECT `id` FROM " . table("channel_group") . " WHERE `standart`='Y' AND `cid`='" . $channel->id() . "'");
			if($query->rows() != 1){
...
class ChannelData{
	...
	function __construct($data){
		$this->data = $data;
		$sql = Database::getInstance()->query("SELECT `uid` FROM " . table("channel_member") . " WHERE `cid`='" . $this->id() . "'");
		while($row = $sql->fetch()){
	...
	function title($new = null){
		if($new != null){
			...
			($database = Database::getInstance())->query("UPDATE " . table("channel") . " SET `title`=" . $database->clean($new) . " WHERE `id`='" . $this->id() . "'");
			$this->data["title"] = $new;
		}
	...
	function leave(UserData $user, $sendMessage = "Leave the channel"){
		if($this->isMember($user)){
			...
			Database::getInstance()->query("DELETE FROM " . table("channel_member") . " WHERE `cid`='" . $this->id() . "' AND `uid`='" . $user->id() . "'");
			...
			if($isOnline){
				Message::send_user($user, $user->nick()."@LEAVE " . $this->name() . ": " . ($sendMessage ? $sendMessage : "Unknown whey"));
			}
		}
	}
	...
	function send($message, UserData $user = null){
		...
		return Message::send_channel($this, $user, $user->nick() . "@" . $message);
	}
	...
	function remove(){
		...
		Database::getInstance()->query("DELETE FROM " . table("message") . " WHERE `cid`='" . $this->id() . "'");
	}
...
class ChannelMember{
	...
	public function __construct(UserData $user, ChannelData $data){
		...
		$this->data = Database::getInstance()->query("SELECT * FROM " . table("channel_member") . " WHERE `cid`='" . $data->id() . "' AND `uid`='" . $user->id() . "'")->fetch();
		$this->channel = $data;
	...
	public function updateActive(){
		...
		Database::getInstance()->query("UPDATE " . table("channel_member") . " SET `active`='" . time() . "'" . $append . " WHERE `id`='" . $this->data["id"] . "'");
	}
	public function markInaktiv(){
		$this->channel->send("INAKTIV " . $this->channel->name() . ": YES", $this->user);
		$this->data["isInaktiv"] = "Y";
		Database::getInstance()->query("UPDATE " . table("channel_member") . " SET `isInaktiv`='Y' WHERE `cid`='" . $this->channel->id() . "' AND `uid`='" . $this->user->id() . "'");
	}
}
class ChannelGroup{
	private $data;
	function __construct(ChannelMember $member){
		...
		$this->data = Database::getInstance()->query("SELECT * FROM " . table("channel_group") . " WHERE `id`='" . $member->groupId() . "'")->fetch();
	}
