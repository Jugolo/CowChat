		...
		if (this.readyState == 4 && this.status == 200) {
			...
			for (var i = 0; i < parts.length; i++) {
				...
				if (parts[i] == "LOGIN: REQUID") {
					location
							.assign("?login=false&page=login.html&defender=enabled");
					return;
				}
				onIncomming(parts[i]);
			}
		...
		if (msg.hasChannel()) {
			...
			switch(msg.command()){
			   case "JOIN":
				   if(msg.nick() != myNick){
					   var channel;
					   if((channel = getChannel(msg.message())) != null){
						   channel.appendHTML(language("%s joined the channel", msg.nick()));
						   channel.appendUser(msg.nick());
						   //wee are done here
						   sendBuffer.flush();
					   }
				   }
				break;
				default:
					console.log(language("Unknown command %s", msg.command()));
			}
		}
	}
}
